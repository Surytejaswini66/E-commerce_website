...
const PlantItemsContainer = () => {
  ...
  return (
    ...
    >
      {/* First Plant Item Column */}
      <div style={{ display: "flex", flexDirection: "column" }}>
        {Array.from({ length: 3 }).map((_, index) => (
          <div key={index} style={{ marginBottom: "20px" }}>
            <PlantItemsList {...plantsData[0]} /> {/* First Plant Item */}
          </div>
        ))}
      </div>

      {/* Second Plant Item Column */}
      <div style={{ display: "flex", flexDirection: "column" }}>
        {Array.from({ length: 3 }).map((_, index) => (
          <div key={index} style={{ marginBottom: "20px" }}>
            <PlantItemsList {...plantsData[1]} /> {/* Second Plant Item */}
          </div>
        ))}
      </div>

      {/* Third Plant Item Column */}
      <div style={{ display: "flex", flexDirection: "column" }}>
        {Array.from({ length: 3 }).map((_, index) => (
          <div key={index} style={{ marginBottom: "20px" }}>
            <PlantItemsList {...plantsData[2]} /> {/* Third Plant Item */}
          </div>
        ))}
      </div>
    </div>
